...
4. **Endpoints** - You can configure the types [Service Endpoints](/docs/services/databases-for-etcd?topic=databases-for-etcd-service-endpoints) supported on your deployment. Its default is that connections to your deployment can be made from the public network.
...
* `service-endpoints` - Selects the types [Service Endpoints](/docs/services/databases-for-etcd?topic=databases-for-etcd-service-endpoints) supported on your deployment. Options are `public`, `private`, or `public-and-private`. If omitted, the default is `public`.