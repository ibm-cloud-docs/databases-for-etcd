...
In order to connect to {{site.data.keyword.databases-for-etcd_full}}, you need credentials and some connection strings. A {{site.data.keyword.databases-for-etcd}} deployment is provisioned with a root user, and after [setting the root password](/docs/services/databases-for-etcd?topic=databases-for-etcd-root-password), you can use its connection strings to connect to your deployment.
